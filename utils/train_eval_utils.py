...
def train_one_epoch(model, optimizer, data_loader, device, epoch,
    ...
    model.train()
    metric_logger = dutils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', dutils.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    ...
    if epoch == 0 and warmup is True:  # 当训练第一轮（epoch=0）时，启用warmup训练方式，可理解为热身训练
        ...
        lr_scheduler = dutils.warmup_lr_scheduler(optimizer, warmup_iters, warmup_factor)
    ...
    for i, [images, targets] in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        loss_dict_reduced = dutils.reduce_dict({"losses": losses})
